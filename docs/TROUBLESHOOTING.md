...
| Picker shows no `AiroMote-…` device | Controller not powered / not advertising (LED should heartbeat). Already connected to another tab or the OS Bluetooth panel — disconnect there. On Linux enable the experimental Web Bluetooth flag. |
...
| Connected but "Not connected" in AiroMote after a page reload | Chrome only auto-reconnects previously permitted devices with `getDevices()` available; otherwise connect again (one click). |
...
| Low frame rate in 3D activities | AiroMote picks a scene quality tier from the GPU (shadows off and lower pixel ratio on integrated GPUs). Force it with `localStorage.setItem('aero.sceneQuality','low')` (or `'high'`) and reload. Close other GPU-heavy tabs. |
...
| Symptom | Fix |
|---|---|
| After `-t upload` the board sits in download mode (`boot:0x4 DOWNLOAD`, no BLE advert) | On boards with the native USB Serial/JTAG port the host's DTR/RTS emulation can leave the chip in download mode after esptool's reset. Press the board's RESET/EN button once (or unplug and replug USB). Verified on ESP32-C6FH4: after a physical reset the app advertises as `AiroMote-1-xxxx`. |
